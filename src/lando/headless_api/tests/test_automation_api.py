...
@pytest.mark.django_db
def test_automation_job_add_commit_success(hg_server, hg_clone, monkeypatch):
...
@pytest.mark.django_db
def test_automation_job_add_commit_fail(hg_server, hg_clone, monkeypatch):
    repo = Repo.objects.create(
        scm_type=SCM_TYPE_HG,
        name="mozilla-central",
        url=hg_server,
    ...
    assert not worker.run_automation_job(job)
